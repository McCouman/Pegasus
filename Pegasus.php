<?php
/**
 * Wikibyte (new Pegasus) - Branch of MonoBook which has many usability improvements and
 * somewhat cleaner code.
 *
 * @Produced by Michael McCouman jr. 
 * Info: Wikinode was renamed on the 17th of October, 2010 by M.C. on Pegasus
 */
...
/**
 * SkinTemplate class for Wikinode skin - Pegasus
 */
class SkinPegasus extends SkinTemplate {

	/* Functions */
	var $skinname = 'pegasus', $stylename = 'pegasus',
		$template = 'pegasusTemplate', $useHeadElement = true;
	...
	public function initPage( OutputPage $out ) {
		...
		$out->addScript(
			'<!--[if lt IE 7]><style type="text/css">body{behavior:url("' .
				$wgLocalStylePath .
				'/pegasus/csshover.htc")}</style><![endif]-->'
		);
	}
	...
	function setupSkinUserCss( OutputPage $out ){
		global $wgpegasusExtraStyles;
		
		parent::setupSkinUserCss( $out );

		// Append skin-specific styles
		$out->addStyle( 'pegasus/main-rtl.css', 'screen', '', 'rtl' );
		$out->addStyle( 'pegasus/main-ltr.css', 'screen', '', 'ltr' );

		// Add extra stylesheets
		// THIS IS ONLY USEFUL FOR EXPERIMENTING WITH DIFFERNT STYLE OPTIONS! THIS WILL BE REMOVED IN THE NEAR FUTURE.
		if ( is_array( $wgpegasusExtraStyles ) ) {
			foreach ( $wgpegasusExtraStyles as $style ) {
				$out->addStyle( 'pegasus/' . $style, 'screen' );
			}
		}
	}
 ...
 global $wgDisableLangConversion, $wgpegasusUseIconWatch;
		...
		if( $this->iscontent ) {
			...
			if ( $this->mTitle->exists() && $userCanRead ) {
				$links['views']['view'] = $this->tabAction(
					$isTalk ? $talkPage : $subjectPage,
						'pegasus-view-view', ( $action == 'view' || $action == 'purge' ), '', true
				);
			}
			...
			if (
				...
				$links['views']['edit'] = array(
					...
					'text' => $this->mTitle->exists()
						? wfMsg( 'pegasus-view-edit' )
						: wfMsg( 'pegasus-view-create' ),
					'href' =>
				...
				if ( ( $isTalk && $wgArticle && $wgArticle->isCurrent() ) || ( $wgOut->showNewSectionLink() ) ) {
					...
					if ( !$wgOut->forceHideNewSectionLink() ) {
						...
						$links['views']['addsection'] = array(
							...
							'text' => wfMsg( 'pegasus-action-addsection' ),
			...
			// Checks if the page has some kind of viewable content
				...
				$links['views']['viewsource'] = array(
					...
					'text' => wfMsg( 'pegasus-view-viewsource' ),
					'href' =>
						$this->mTitle->getLocalURL( $this->editUrlOptions() )
				);
			}
			...
			if ( $this->mTitle->exists() && $userCanRead ) {
				...
				$links['views']['history'] = array(
					...
					'text' => wfMsg( 'pegasus-view-history' ),
				...
				if( $wgUser->isAllowed( 'delete' ) ) {
					$links['actions']['delete'] = array(
						...
						'text' => wfMsg( 'pegasus-action-delete' ),
				...
				if ( $this->mTitle->quickUserCan( 'move' ) ) {
					...
					$links['actions']['move'] = array(
						'class' => $this->mTitle->isSpecial( 'Movepage' ) ?
										'selected' : false,
						'text' => wfMsg( 'pegasus-action-move' ),
					...
					if ( !$this->mTitle->isProtected() ) {
						$links['actions']['protect'] = array(
							'class' => ( $action == 'protect' ) ?
											'selected' : false,
							'text' => wfMsg( 'pegasus-action-protect' ),
							'href' =>
						...
						$links['actions']['unprotect'] = array(
							'class' => ( $action == 'unprotect' ) ?
											'selected' : false,
							'text' => wfMsg( 'pegasus-action-unprotect' ),
							'href' =>
			 ...
			 * the pegasus skin, would break the Ajax code which makes this
...
/**
 * QuickTemplate class for Wikinode skin (new Pegasus)
 * @ingroup Skins
 */
class PegasusTemplate extends QuickTemplate {
...
?>
		...
		<div id="head-grey" class="noprint"><center><h1 id="firstHeading" class="firstHeading"><?php $this->html( 'title' ) ?></h1></center></div>
<!-- content -->
	<div id="content"<?php $this->html('specialpageattributes') ?>>
		...
		<div id="head" class="noprint">
			...
			<div id="left-navigation">
				<?php $this->renderNavigation( array( 'NAMESPACES', 'VIEWS', 'ACTIONS', 'VARIANTS' ) ); ?>
			</div>
...
<!-- Werbung werber------>
	<!-- MarjorieWiki
		<a style="color:#fff; margin-left:-4px; margin-top:-12px;" href="http://marjorie-wiki.org" target="_blank">
			<img style="margin-top:-5px;" height="47" width="165" src="http://wikinode.square7.ch/w/extensions/wikinodeextem/Werber/MW-mini.png" alt="Marjorie.png" text="http://marjorie-wiki.org">
		</a>
	  /MarjorieWiki-->
<!-- /Werbung werber ende------>
			</div>
		...
		<div id="footer"<?php $this->html('userlangattributes') ?>>
			...
			<div style="border: #eee solid 1px; background: #eee;"><center>
			 <small>
			  Powered by 
			   <a herf="c3ltax">C3ltaX Media</a> | Produced by 
			    <a href="http://de.community.wikiunity.com/wiki/Benutzer:McCouman" text="Michael McCouman junior">
...
<?php
	...
	private function renderNavigation( $elements ) {
		global $wgContLang, $wgpegasusUseSimpleSearch, $wgpegasusShowVariantName, $wgStylePath;
		...
		foreach ( $elements as $name => $element ) {
			...
			switch ( $element ) {
				case 'NAMESPACES':
?>
<div id="p-namespaces" class="pegasusTabs<?php if ( count( $this->data['namespace_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<?php
				...
				case 'VARIANTS':
?>
<div id="p-variants" class="pegasusMenu<?php if ( count( $this->data['variant_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
	<?php if ( $wgpegasusShowVariantName ): ?>
		<h4>
...
<?php
				...
				case 'VIEWS':
?>
<div id="p-views" class="pegasusTabs<?php if ( count( $this->data['view_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<?php
				...
				case 'ACTIONS':
?>
<div id="p-cactions" class="pegasusMenu<?php if ( count( $this->data['action_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
...
<div id="p-search">
	...
	<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
		...
		<?php if ( $wgpegasusUseSimpleSearch ): ?>
		<div id="simpleSearch">
